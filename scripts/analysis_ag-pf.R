...
unweighted_vsumcorr <- ggplot(df[N,], aes(x=dists, y=mags)) + theme_classic()+
     geom_jitter(color=aglight, alpha=1/(df[N,]$dists/500), width=0.1, height=0.1) + 
...
weighted_vsumcorr <- ggplot(df[N,], aes(x=dists, y=mags)) + theme_classic()+
     geom_jitter(color=pflight, alpha=1/(df[N,]$dists/500), width=0.1, height=0.1) + 
     geom_smooth(method='lm', color=pfdark, fill=pfdark) +scale_y_continuous(trans='log10') +scale_x_continuous(trans='log10') +
     ylab('Magnitude of vector sum (km)') + xlab('Distance between samples (km)')
