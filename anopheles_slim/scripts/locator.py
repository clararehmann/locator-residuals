...
def load_genotypes(samples):
    if args.zarr is not None:
        ...
        idx = []
        count = 0
        for ID in callset['samples'].astype('str'):
            if ID in samples:
                idx.append(count)
            count += 1
        genotypes = allel.GenotypeArray(gt[idx])
        samples = callset['samples'][idx]
...
def sort_samples(samples):
    ...
    sample_data.sampleID = metadata.sampleID.astype(str)   # make sure sampleIDs are strings
...
def train_network(model,traingen,testgen,trainlocs,testlocs):
    history = model.fit(x=traingen,y=trainlocs,
...
if args.windows:
    ...
    metadata.sampleID = metadata.sampleID.astype(str)   # make sure sampleIDs are strings
    ...
    for i in np.arange(start,stop,size):
        ...
        print(a,b)
        gw=allel.GenotypeArray(gt[a:b])
        genotypes=gw.take(samples_callset_index, axis=1)
